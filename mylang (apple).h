...
class Display
{
	public:
		string a[15][15];
		int obj_x,obj_y;
		int x,y,nrow,ncolumn,i,j,steps=0;
...
int Display::obj()
{
	...
	{
		show();
		cout<<"Steps to reach apple: "<<steps<<endl; sleep(2 * t_factor);
	}
	steps=0;
	obj_x=rand()%13 + 1;
	obj_y=rand()%13 + 1;
...
int Display::show()
{
	if(inst==true)
	{
		...
		cout<<"Reach the apple in lowest steps possible."<<endl<<endl;
		cout<<"CONTROLS"<<endl;
